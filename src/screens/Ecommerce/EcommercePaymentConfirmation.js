...
const transactionSuccessText =
  'It will be settled in a few moments, depending on Ethereum network load';
...
class SendTokenTransaction extends React.Component<Props, State> {
  constructor(props) {
    ...
    this.state = {
      noteSent: false,
      orderSent: false,
      ecommOrderBundle: this.props.navigation.state.params.ecommOrderBundle,
    };
  }
  ...
  componentDidUpdate() {
    ...
    const {
      isSuccess, note, to, txHash, ecommOrderBundle, usdVal, ethVal, plrVal, shippingAddr1, shippingAddr2, customerWallet, customerName,
    } = navigation.state.params;
    if (isSuccess) {
      if(!this.state.orderSent) {
        this.makeRemoteRequestForPushingTxHashAndOrderObject(txHash, ecommOrderBundle, usdVal, ethVal, plrVal, shippingAddr1, shippingAddr2, customerWallet, customerName);
      }
    }
  }

  makeRemoteRequestForPushingTxHashAndOrderObject = (txHash, ecommOrderBundle, usdVal, ethVal, plrVal, shippingAddr1, shippingAddr2, customerWallet, customerName) => {
    let cartFeedSerialized = btoa(JSON.stringify(ecommOrderBundle));
    return fetch('https://devagg.xyz/api/set_order?cart_object='+cartFeedSerialized+'&usd_price='+usdVal+'&eth_price='+ethVal+'&plr_price='+plrVal+'&customer_ship_addr_1='+shippingAddr1+'&customer_ship_addr_2='+shippingAddr2+'&customer_wallet_addr='+customerWallet+'&customer_name='+customerName+'&trx_id='+txHash)
    .then((response) => response.json())
    .then((responseJson) => {
      this.setState({
        isLoading: false,
        orderSent: true,
        inAggregator: responseJson.status,
      }, function(){});
    })
    .catch((error) =>{
      console.error(error);
  ...
  render() {
    const { navigation } = this.props;
    const { isSuccess, txHash, error } = navigation.state.params;
    ...
    const transactionStatusTitle = isSuccess ? 'Order placed' : 'Transaction failed';
    return (
      <Container>
